...
void MainWindow::deleteaccount(int i)
{
    ...
    if (result == QMessageBox::Yes) {
        ...
        //删除本地文件夹记录
        QString accountToDelete = infoList.at(indexToRemove);
        QString dirToDelete = QCoreApplication::applicationDirPath() + "//..//UserFile//" + accountToDelete ;
        QDir dir(dirToDelete);
        if (dir.exists()) {
            if (!dir.removeRecursively()) { // 递归删除文件夹及其内容
                qDebug() << "删除文件夹失败：" << dirToDelete;
            } else {
                qDebug() << "成功删除文件夹：" << dirToDelete;
            }
        }
        // 删除账号和密码
