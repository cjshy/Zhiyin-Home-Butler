...
qint64 Search::timeToQint(const QString &time)
{
    QStringList parts = time.split(":");
       ...
       // 计算总秒数并转换为 qint64
       return static_cast<qint64>(minutes * 60 + seconds);
}
...
void Search::on_tableView_doubleClicked(const QModelIndex &index)
{
    ...
    _song.Time = timeToQint(duration); // 假设 duration 是字符串形式的整数
    _song.music_id = music_id.toInt();
    mainWidow->song_List.push_back(_song);
    qDebug()<<"___时长————"<<_song.Time;
    // 构建歌曲播放链接
