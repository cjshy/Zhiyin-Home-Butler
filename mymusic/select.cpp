    ...
    ui(new Ui::Select),mainWidow(mainWidow)
{
    ...
    // 创建图标
    QIcon onlineSearchIcon(":/tu/搜索.png"); // 替换为您的图标路径
    QIcon myFavoritesIcon(":/tu/爱心.png"); // 替换为您的图标路径
    QIcon recentlyPlayedIcon(":/tu/最近播放.png"); // 替换为您的图标路径

    // 创建项并设置图标和文本
    QStandardItem *onlineSearchItem = new QStandardItem(onlineSearchIcon, "在线搜索");
    QStandardItem *myFavoritesItem = new QStandardItem(myFavoritesIcon, "我的喜欢");
    QStandardItem *recentlyPlayedItem = new QStandardItem(recentlyPlayedIcon, "最近播放");

    // 将项添加到模型中
    model->appendRow(onlineSearchItem);
    model->appendRow(myFavoritesItem);
    model->appendRow(recentlyPlayedItem);
    ...
    recent->hide();
    word = new song_word(mainWidow,ui->frame);
    word->hide();
    // 初始化音量滑块
    ...
    m_liebiao->setFocusPolicy(Qt::NoFocus);
...
void Select::on_listView_clicked(const QModelIndex &index) // 选择功能
{
    ...
    if (!item) return; // 确保项有效

    // 获取文本
    QString ret = item->text(); // 直接获取文本
    ...
    if (ret == "在线搜索") {
        ...
        mylove->show();
        mylove->send_singal();    // 发出信号
        mylove->showMy_love();    // 显示我的喜欢
        mainWidow->love_list.clear(); // 清空列表
    }
    else if (ret == "最近播放") {
        recent->show();
        recent->showTable(); // 显示最近播放的表格
    }
}
...
void Select::showPlaylistWithAnimation()//播放列表的出场动画
{
    // 设置动画
...
void Select::hidePlaylistWithAnimation()//播放列表的退场动画
{
    // 设置动画
    QPropertyAnimation *geometryAnimation = new QPropertyAnimation(widget, "geometry");
...
void Select::on_pushButton_clicked()//歌词界面显示
{
    search->hide();
    mylove->hide();
    recent->hide();
    ...
    if (currentIndex >= 0 && currentIndex < mainWidow->song_List.size()) {
        MainWindow::song currentMainSong = mainWidow->song_List[currentIndex]; // 获取当前播放的歌曲信息
        qDebug()<<"歌曲名为："<<currentMainSong.songname<<"歌曲id为："<<currentMainSong.music_id;
        QString url = QString("http://music.163.com/api/song/lyric?id=%1&lv=1&kv=1&tv=-1").arg(currentMainSong.music_id);

        word->manager->get(QNetworkRequest(QUrl(url)));
    } else {
        qDebug() << "没有这首歌";
    }
    if(word->isHidden())
